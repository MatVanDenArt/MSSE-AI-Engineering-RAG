...
with st.sidebar:
    ...
    if "actual_provider" not in st.session_state:
        st.session_state.actual_provider = "Groq"
        
    provider_options = ["Groq", "Gemini"]
    # Default the selectbox to the actual_provider
    provider_idx = provider_options.index(st.session_state.actual_provider)
    user_selected = st.selectbox("LLM Provider", provider_options, index=provider_idx)
    
    # Sync the session state with whatever the user just clicked
    st.session_state.actual_provider = user_selected
    model_provider = st.session_state.actual_provider
        ...
        with st.spinner("Searching policies..."):
            try:
                ...
                if used_provider != model_provider:
                    ...
                    # Automatically switch the working model for future queries without mutating a widget key
                    st.session_state.actual_provider = used_provider
